...
function generateWav() {
  ...
  const totalDuration = 9.0; // 9 seconds duration
  ...
  // 5 Waves of chimes: at 0.0s, 1.8s, 3.6s, 5.4s, 7.2s
  ...
  bursts.forEach((burstTime) => {
    ...
    // Note 3: 1760Hz (A6)
    addTone(samples, sampleRate, burstTime + 0.40, 1760, 0.90, 0.38);
  });
  ...
  const outPathWav = path.join(__dirname, '../public/sounds/new-order.wav');
  fs.writeFileSync(outPathWav, buffer);
  console.log('Successfully written', outPathWav, buffer.length, 'bytes');

  // Also write as new-order.mp3 fallback alias
  const outPathMp3 = path.join(__dirname, '../public/sounds/new-order.mp3');
...
function addTone(samples, sampleRate, startTime, freq, duration, amplitude) {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    if (t < 0.015) {
      ...
      // Exponential decay
      env = Math.exp(-4.5 * (t - 0.015) / duration);
    }
